...
export class Game {
  ...
  async fetchGamePksForCurrentDate() {
    const currentDate = new Date();
    const formattedDate = currentDate.toISOString().split('T')[0]; // Format: 'YYYY-MM-DD'
    return this.fetchGamePksForDate(formattedDate);
  }

  async fetchGamePksForDate(date: string) {
    ...
    try {
      ...
      // Extract gamePks from the games array
      const gamePks = games.map((game: { gamePk: string; }) => game.gamePk);
      
      return gamePks;
    } catch (error) {
      console.error('Error fetching game Pks:', error);
      return [];
    }
  }

  async fetchGameData(gamePks: string[]) {
    const gameDataArray = [];

    for (const gamePk of gamePks) {
      const gameData = await this.fetchGameDataForGame(gamePk);
  ...
  async fetchGameDataForGame(gamePk: string) {
    const endpoint = `https://statsapi.mlb.com/api/v1.1/game/${gamePk}/feed/live`;
